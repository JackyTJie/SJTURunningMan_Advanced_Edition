...
<div align="center">
  ...
  <p><strong>轻量 · 实际路线预览 · 记录生成与上传流程研究</strong></p>
...
- **跑步距离**: 直接输入距离，单位为km，最大不超过5km（默认4km）
...
- **预览路线按钮**: 上传前按当前设置在本地离线生成补点路线并在系统浏览器中预览；上传中/后打开本条实际上传轨迹
...
- 点击“预览路线”即按当前距离/路线设置在本地跑一遍生成算法（补点、分段、时间戳），无需登录
- 生成结果在系统浏览器的 OpenStreetMap 地图中展示：路线连线、绿色起点、红色终点

**实际上传路线预览:**

- 上传流程中/结束后，预览使用已生成并准备上传的 payload 坐标，而不是原始 txt 路线
...
2. **生成并上传跑步数据**: 逐条根据配置生成符合要求的跑步轨迹数据并上传到服务器
...
3. **设置上传参数**:
   ...
   - 在"跑步距离"中输入距离，单位为 km，最多 5km
   - 在"预设路线"中选择内置 `default` 路线，或选择"自定义..."导入 txt 路线文件

4. **开始上传**:
   - 点击"开始生成并上传"按钮开始上传
   - 观察进度条、日志输出和路线预览摘要了解上传状态
   - 如需查看本条实际上传轨迹，点击"预览路线"
   - 如需停止，点击"停止任务"按钮
